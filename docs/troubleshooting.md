...
- scheduled expiry enforcement runs without Apple credentials. Full missed-notification repair requires `APPLE_SERVER_API_SECRET_ARN` to reference a Secrets Manager JSON value containing `private_key`, `key_id`, and `issuer_id`; inspect `reconciliation_error`, retry count and next reconciliation time in root admin if the Apple status lookup fails
