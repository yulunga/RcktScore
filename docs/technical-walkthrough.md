...
- tennis supports normal advantage scoring or receiver-choice No-Ad scoring, regular seven-point tiebreaks at 6-6, and an optional deciding-set replacement played as a 10-point match tiebreak; all tiebreaks require a two-point margin
...
- `match_settings`
- `timer`
- tennis uses `receiver_choice` at a No-Ad deuce and does not accept squash-only `let`, `stroke`, or `serve_side` actions
...
3. [DashboardView.swift](/Users/glennrowe/Development/Projects/RcktScore/mobile/ios/RcktScoreMobile/RcktScoreMobile/Views/DashboardView.swift) loads `GET /dashboard/{organization_id}`. Personal Free presents `Home`, `Matches`, `History`, `Settings`, and `Need Help`, with three readable history records and a locked fourth teaser. Personal Plus replaces the standalone History tab with Performance; Matches opens directly and presents a segmented Current Matches / Match History selector at the top. The header bell becomes an offline indicator while disconnected. Online, it loads the persisted notification inbox and glows yellow while any message is unread. A cached active match can be reopened from the active-match section.
...
14. [StoreKitPurchaseService.swift](/Users/glennrowe/Development/Projects/RcktScore/mobile/ios/RcktScoreMobile/RcktScoreMobile/Services/StoreKitPurchaseService.swift) loads the monthly and yearly Personal Plus products, displays Apple's localised prices, observes transaction updates, reads current entitlements, restores purchases, and supports Apple's manage-subscriptions sheet. The subscription page expands neutral, consistently styled monthly/yearly choices directly below Personal Plus for 20 seconds, then slowly collapses them. StoreKit entitlements are refreshed when the page opens, when the app returns active and after subscription management closes, but the displayed current account tier comes from the latest backend dashboard/settings plan rather than a local test transaction. The current card keeps the standard body background, gains a green outline and displays its Current badge in pink. Debug purchases still do not change the backend account plan or server-side entitlements. Release purchasing remains disabled until server verification is implemented.
...
- migration `025_apple_subscription_account_identity.sql` and `GET /subscriptions/apple/context/{organization_id}` now provide the stable server-owned account UUID and entitlement-audit foundation; iOS has not yet been connected to that endpoint and transaction JWS verification remains the next implementation stage
